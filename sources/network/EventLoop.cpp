...
/*   Updated: 2026/06/20 14:35:48 by lcluzan          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void EventLoop::registerCgi(int client_fd, const t_httpResponse& response, const t_httpRequest& req, const ServerConfig& config) {
    ...
    pfd_read.events = POLLIN; // Wait for CGI script to write something
    pfd_read.revents = 0;
...
void EventLoop::handleCgiRead(int fd, CgiState& CgiContext) {
    ...
    if (bytes > 0) {
		...
		waitpid(CgiContext.cgi_pid, NULL, WNOHANG);
        ...
        // 3. Formatted HTTP/1.1 Response
        std::stringstream ss;
        ss << "HTTP/1.1 200 OK\r\n";

        // Append headers provided by CGI script (like Content-Type)
        ...
        // HTML/Text body generated by the script
        ss << cgi_body;

        // 4. Queue formatted string for client
        queueResponse(CgiContext.client_fd, ss.str());

        // 5. Cleanup pipe and remove from poll
        close(fd);
		...
		_cgi_contexts.erase(_pipe_client_fds[fd]); // Pass the client_fd value
		_pipe_client_fds.erase(fd);
    }
}

void	EventLoop::handleCgiEvent(int fd, short revents) {
	// 1. Find which client this is from
	int	client_fd = _pipe_client_fds[fd];

	// 2. Grab the state reference
...
void	EventLoop::checkCgiTimeout() {
	...
	while (it != _cgi_contexts.end()) {
		if (std::difftime(currentTime, it->second.start_time) > 5.0) {
			...
			// 4. Erase context and safely increment iterator
